...
import {SLIDER_IMAGE_ACTION} from '../../interfaces/enums';
...
export function Home(props: ComponentProp) {
  ...
  const [sliderDataSource, setSliderSliderDataSource] = useState(
    [] as ImageSliderDataSource[],
  );
  useEffect(() => {
    getSliders();
  }, []);
  ...
  const getSliders = async () => {
    ...
    if (sliderResp.success) {
      const sliderImages: ImageSliderDataSource[] = sliderResp.data.map(
        (item) => {
          return {
            url: getImageLink(
              item.sliderImage.optimizedDestinationPath,
            ) as string,
          };
        },
      );
  ...
  const onPressImage = (sliderData: ISlider) => {
    const {routeName, navigationParams} = getNavigationPramsFromImageSlider(
      sliderData,
    );
    if (routeName) {
    ...
    <Container>
      <ScrollView>
        <ImageSlider
          ...
          onPress={(position: ImageOnPress) =>
            onPressImage(sliders[position.index])
          }
        />
